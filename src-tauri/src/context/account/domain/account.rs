...
impl Account {
    ...
    /// FEE-028 — what a one-off management fee removes when it is entered as the quantity
    /// `asset_id` should hold after it, as of `date`: the held quantity minus the resulting
    /// one, exactly. Rejects a date outside the transaction bounds (TRX-020) before
    /// replaying, then a resulting quantity below zero or not strictly below the held
    /// quantity.
    pub fn management_fee_removal(
        &self,
        asset_id: &str,
        date: &str,
        ...
        if resulting_quantity < 0 {
            return Err(AccountError::ResultingQuantityNegative);
        }
        let held_quantity = self.holding_quantity_as_of(asset_id, date);
        if resulting_quantity >= held_quantity {
            return Err(AccountError::ResultingQuantityNotBelowHeld { held_quantity });
        }
        let removed_quantity = held_quantity - resulting_quantity;
        let percent_micros =
            (removed_quantity as i128 * 100_000_000 / held_quantity as i128) as i64;
        Ok(ManagementFeeRemoval {
            held_quantity,
            removed_quantity,
            percent_micros,
        })
